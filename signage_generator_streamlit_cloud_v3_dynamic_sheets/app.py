import copy
from datetime import date, datetime, time, timedelta
from io import BytesIO
from typing import Dict, List, Optional, Tuple

import openpyxl
import pandas as pd
import streamlit as st
from openpyxl.worksheet.worksheet import Worksheet

HEADER_KEYWORDS = {
    "date": ["날짜", "일자"],
    "alarm": ["알람", "알림"],
    "start": ["시작예정", "시작 예정", "시작시간", "시작 시각", "시작"],
    "program": ["프로그램명", "프로그램"],
    "room": ["렉처룸", "렉쳐룸", "강의실", "룸"],
}
TIME_FORMAT = "hh:mm"
DATE_FORMAT = "m월 d일"
DEFAULT_TARGET_SHEET = "스케줄"


def normalize(v) -> str:
    return str(v).replace(" ", "").strip().lower() if v is not None else ""


def cell_has_keyword(value, keywords: List[str]) -> bool:
    text = normalize(value)
    return any(normalize(k) in text for k in keywords)


def get_workbook_bytes(uploaded_file) -> bytes:
    if uploaded_file is None:
        return b""
    return uploaded_file.getvalue()


def detect_source_sheets(uploaded_file, target_sheet_name: str) -> List[str]:
    """업로드한 엑셀에서 출력 시트(기본: 스케줄)를 제외한 모든 시트명을 반환합니다."""
    if uploaded_file is None:
        return []
    wb = openpyxl.load_workbook(BytesIO(get_workbook_bytes(uploaded_file)), read_only=True, data_only=False)
    target = (target_sheet_name or "").strip()
    if not target:
        # 출력 시트명을 비운 경우 첫 번째 시트를 출력 시트로 간주합니다.
        target = wb.sheetnames[0] if wb.sheetnames else DEFAULT_TARGET_SHEET
    return [name for name in wb.sheetnames if name != target]


def find_header(ws: Worksheet, max_scan_rows: int = 40) -> Tuple[int, Dict[str, int]]:
    best_row = 1
    best_cols: Dict[str, int] = {}
    best_score = 0
    scan_rows = min(ws.max_row, max_scan_rows)
    for row_idx in range(1, scan_rows + 1):
        cols: Dict[str, int] = {}
        for cell in ws[row_idx]:
            for key, keywords in HEADER_KEYWORDS.items():
                if key not in cols and cell_has_keyword(cell.value, keywords):
                    cols[key] = cell.column
        score = len(cols)
        if score > best_score:
            best_row = row_idx
            best_cols = cols
            best_score = score
    if best_score < 3:
        raise ValueError(f"'{ws.title}' 시트에서 날짜/알람/시작/프로그램/렉처룸 헤더를 찾지 못했습니다.")
    return best_row, best_cols


def row_has_schedule_data(ws: Worksheet, row_idx: int, cols: Dict[str, int]) -> bool:
    program_col = cols.get("program")
    start_col = cols.get("start")
    room_col = cols.get("room")
    program = ws.cell(row_idx, program_col).value if program_col else None
    start = ws.cell(row_idx, start_col).value if start_col else None
    room = ws.cell(row_idx, room_col).value if room_col else None
    return bool(program and start and room)


def source_data_rows(ws: Worksheet, header_row: int, cols: Dict[str, int]) -> List[int]:
    return [r for r in range(header_row + 1, ws.max_row + 1) if row_has_schedule_data(ws, r, cols)]


def last_used_row(ws: Worksheet, cols: Optional[Dict[str, int]] = None) -> int:
    min_col = min(cols.values()) if cols else 1
    max_col = max(cols.values()) if cols else ws.max_column
    for r in range(ws.max_row, 0, -1):
        for c in range(min_col, max_col + 1):
            if ws.cell(r, c).value not in (None, ""):
                return r
    return 1


def clear_schedule_body(ws: Worksheet, header_row: int, cols: Dict[str, int]) -> None:
    start_row = header_row + 1
    max_col = max(ws.max_column, max(cols.values()))
    for r in range(start_row, ws.max_row + 1):
        for c in range(1, max_col + 1):
            ws.cell(r, c).value = None


def copy_row_style_and_values(src_ws: Worksheet, dst_ws: Worksheet, src_row: int, dst_row: int, max_col: int) -> None:
    dst_ws.row_dimensions[dst_row].height = src_ws.row_dimensions[src_row].height
    for c in range(1, max_col + 1):
        src = src_ws.cell(src_row, c)
        dst = dst_ws.cell(dst_row, c)
        dst.value = src.value
        if src.has_style:
            dst._style = copy.copy(src._style)
        if src.number_format:
            dst.number_format = src.number_format
        if src.comment:
            dst.comment = copy.copy(src.comment)
        if src.hyperlink:
            dst._hyperlink = copy.copy(src.hyperlink)


def copy_column_widths(src_ws: Worksheet, dst_ws: Worksheet, max_col: int) -> None:
    for c in range(1, max_col + 1):
        letter = openpyxl.utils.get_column_letter(c)
        width = src_ws.column_dimensions[letter].width
        if width:
            dst_ws.column_dimensions[letter].width = width


def to_time_value(v):
    if isinstance(v, datetime):
        return v.time().replace(second=0, microsecond=0)
    if isinstance(v, time):
        return v.replace(second=0, microsecond=0)
    if isinstance(v, (int, float)):
        minutes = round(float(v) * 24 * 60)
        return (datetime(1899, 12, 30) + timedelta(minutes=minutes)).time().replace(second=0, microsecond=0)
    if isinstance(v, str):
        s = v.strip()
        for fmt in ["%H:%M", "%H:%M:%S"]:
            try:
                return datetime.strptime(s, fmt).time()
            except ValueError:
                pass
    return v


def calc_alarm(start_value):
    t = to_time_value(start_value)
    if isinstance(t, time):
        dt = datetime.combine(date(2000, 1, 1), t) - timedelta(minutes=5)
        return dt.time().replace(second=0, microsecond=0)
    return None


def format_date_cell(cell, d: date) -> None:
    cell.value = datetime(d.year, d.month, d.day)
    cell.number_format = DATE_FORMAT


def get_target_sheet(wb, target_sheet_name: str):
    name = target_sheet_name.strip()
    if name:
        if name not in wb.sheetnames:
            raise ValueError(f"'{name}' 시트를 찾지 못했습니다. 출력 시트명을 확인하세요.")
        return wb[name]
    return wb.worksheets[0]


def build_workbook(uploaded_file, schedule_items: List[Dict], clear_existing: bool, target_sheet_name: str) -> Tuple[BytesIO, List[Dict[str, str]]]:
    wb = openpyxl.load_workbook(BytesIO(get_workbook_bytes(uploaded_file)))
    target_ws = get_target_sheet(wb, target_sheet_name)
    target_header_row, target_cols = find_header(target_ws)
    if clear_existing:
        clear_schedule_body(target_ws, target_header_row, target_cols)
        append_row = target_header_row + 1
    else:
        append_row = last_used_row(target_ws, target_cols) + 1
    logs: List[Dict[str, str]] = []
    source_cache: Dict[str, Tuple[int, Dict[str, int], List[int]]] = {}

    for item in schedule_items:
        d = item.get("date")
        sheet_name = str(item.get("sheet", "")).strip()
        if not d or not sheet_name:
            continue
        if isinstance(d, pd.Timestamp):
            d = d.date()
        elif isinstance(d, datetime):
            d = d.date()
        if sheet_name not in wb.sheetnames:
            raise ValueError(f"'{sheet_name}' 시트가 없습니다. 업로드한 엑셀의 시트명을 확인하세요.")
        if sheet_name == target_ws.title:
            raise ValueError(f"'{sheet_name}'은 출력 시트입니다. 타임테이블 원본 시트로 선택할 수 없습니다.")

        src_ws = wb[sheet_name]
        if sheet_name not in source_cache:
            header_row, src_cols = find_header(src_ws)
            rows = source_data_rows(src_ws, header_row, src_cols)
            if not rows:
                raise ValueError(f"'{sheet_name}' 시트에서 복사할 타임테이블 행을 찾지 못했습니다.")
            source_cache[sheet_name] = (header_row, src_cols, rows)
        _, src_cols, rows = source_cache[sheet_name]
        max_col = max(src_ws.max_column, target_ws.max_column, max(target_cols.values()), max(src_cols.values()))
        copy_column_widths(src_ws, target_ws, max_col)
        start_append_row = append_row
        for src_row in rows:
            copy_row_style_and_values(src_ws, target_ws, src_row, append_row, max_col)
            if "date" in target_cols:
                format_date_cell(target_ws.cell(append_row, target_cols["date"]), d)
            if "alarm" in target_cols and "start" in target_cols:
                alarm = calc_alarm(target_ws.cell(append_row, target_cols["start"]).value)
                if alarm is not None:
                    target_ws.cell(append_row, target_cols["alarm"]).value = alarm
                    target_ws.cell(append_row, target_cols["alarm"]).number_format = TIME_FORMAT
            if "start" in target_cols:
                target_ws.cell(append_row, target_cols["start"]).number_format = TIME_FORMAT
            append_row += 1
        logs.append({
            "날짜": d.strftime("%Y-%m-%d"),
            "선택 시트": sheet_name,
            "입력 행수": str(append_row - start_append_row),
        })
    if target_ws.auto_filter:
        target_ws.auto_filter.ref = None
    output = BytesIO()
    wb.save(output)
    output.seek(0)
    return output, logs


def init_items():
    if "schedule_items" not in st.session_state:
        st.session_state.schedule_items = [{"date": date(2026, 4, 1), "sheet": ""}]


def add_item(default_sheet: str):
    items = st.session_state.schedule_items
    next_date = items[-1]["date"] + timedelta(days=1) if items and items[-1].get("date") else date.today()
    items.append({"date": next_date, "sheet": default_sheet})


def remove_item(idx: int, default_sheet: str):
    if 0 <= idx < len(st.session_state.schedule_items):
        st.session_state.schedule_items.pop(idx)
    if not st.session_state.schedule_items:
        st.session_state.schedule_items.append({"date": date.today(), "sheet": default_sheet})


def coerce_items_to_options(options: List[str]):
    if not options:
        return
    for item in st.session_state.schedule_items:
        if item.get("sheet") not in options:
            item["sheet"] = options[0]


st.set_page_config(page_title="HMG 사이니지 스케줄 생성기", layout="wide")
st.title("HMG 사이니지 스케줄 생성기")
st.caption("업로드한 엑셀에서 출력 시트를 제외한 모든 시트를 자동 감지해 드롭다운에 표시합니다.")
init_items()

with st.sidebar:
    st.header("1. 기준 엑셀")
    uploaded = st.file_uploader("타임테이블 시트가 들어있는 엑셀 업로드", type=["xlsx"])
    target_sheet_name = st.text_input("출력 시트명", value=DEFAULT_TARGET_SHEET, help="이 시트를 제외한 모든 시트가 선택 목록에 표시됩니다. 비우면 첫 번째 시트를 출력 시트로 사용합니다.")
    clear_existing = st.checkbox("기존 스케줄 내용 삭제 후 새로 생성", value=True)

try:
    sheet_options = detect_source_sheets(uploaded, target_sheet_name) if uploaded else []
except Exception as e:
    sheet_options = []
    st.error(f"시트 목록을 읽지 못했습니다: {e}")

if uploaded:
    if sheet_options:
        coerce_items_to_options(sheet_options)
        with st.sidebar:
            st.divider()
            st.header("자동 감지된 타임테이블 시트")
            st.write(f"총 {len(sheet_options)}개")
            st.dataframe(pd.DataFrame({"시트명": sheet_options}), use_container_width=True, hide_index=True)
    else:
        st.warning("출력 시트를 제외한 타임테이블 시트가 없습니다. 출력 시트명을 확인하세요.")

st.subheader("2. 날짜별 타임테이블 선택")
if not uploaded:
    st.info("먼저 기준 엑셀 파일을 업로드하면, 출력 시트를 제외한 모든 시트가 자동으로 드롭다운에 표시됩니다.")
elif not sheet_options:
    st.info("선택 가능한 타임테이블 시트가 없습니다.")
else:
    st.write("시트를 추가하거나 이름을 바꿔도, 업로드한 엑셀 기준으로 선택 목록이 자동 갱신됩니다.")

for i, item in enumerate(st.session_state.schedule_items):
    c1, c2, c3 = st.columns([1.3, 1.8, 0.6])
    with c1:
        st.session_state.schedule_items[i]["date"] = st.date_input(
            "날짜",
            value=item.get("date") or date.today(),
            key=f"date_{i}",
        )
    with c2:
        if sheet_options:
            current_sheet = item.get("sheet") if item.get("sheet") in sheet_options else sheet_options[0]
            st.session_state.schedule_items[i]["sheet"] = st.selectbox(
                "타임테이블 시트",
                options=sheet_options,
                index=sheet_options.index(current_sheet),
                key=f"sheet_{i}",
            )
        else:
            st.text_input("타임테이블 시트", value=item.get("sheet", ""), disabled=True, key=f"sheet_disabled_{i}")
    with c3:
        st.write("")
        st.write("")
        if st.button("삭제", key=f"del_{i}", use_container_width=True):
            remove_item(i, sheet_options[0] if sheet_options else "")
            st.rerun()

c_add, c_clear = st.columns([1, 1])
with c_add:
    if st.button("날짜 추가", type="secondary", use_container_width=True, disabled=not bool(sheet_options)):
        add_item(sheet_options[0] if sheet_options else "")
        st.rerun()
with c_clear:
    if st.button("전체 초기화", use_container_width=True):
        st.session_state.schedule_items = [{"date": date.today(), "sheet": sheet_options[0] if sheet_options else ""}]
        st.rerun()

preview = pd.DataFrame([
    {"날짜": item["date"].strftime("%Y-%m-%d") if item.get("date") else "", "적용 시트": item.get("sheet", "")}
    for item in st.session_state.schedule_items
])
st.subheader("3. 적용 예정 미리보기")
st.dataframe(preview, use_container_width=True, hide_index=True)

if st.button("엑셀 생성", type="primary", use_container_width=True, disabled=not bool(sheet_options)):
    valid_items = [item for item in st.session_state.schedule_items if item.get("date") and item.get("sheet")]
    if uploaded is None:
        st.error("기준 엑셀 파일을 먼저 업로드하세요.")
    elif not valid_items:
        st.error("날짜와 타임테이블 시트를 최소 1개 이상 입력하세요.")
    else:
        try:
            output, logs = build_workbook(uploaded, valid_items, clear_existing, target_sheet_name)
            st.success("생성 완료")
            st.dataframe(pd.DataFrame(logs), use_container_width=True, hide_index=True)
            st.download_button(
                "완성 엑셀 다운로드",
                data=output,
                file_name="signage_schedule_generated.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                use_container_width=True,
            )
        except Exception as e:
            st.error(str(e))
            st.info("출력 시트명과 헤더명(날짜/알람시간/시작예정시각/프로그램명/렉처룸)을 확인하세요.")
